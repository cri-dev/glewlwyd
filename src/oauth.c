  ...
  if (check_result_value(j_auth, G_OK)) {
    ...
    if (check_result_value(j_validate, G_OK)) {
      ...
      if (check_result_value(j_user, G_OK)) {
        ...
        time(&now);
        refresh_token = generate_refresh_token(config, request->auth_basic_user, json_string_value(json_object_get(json_object_get(j_user, "user"), "login")), json_string_value(json_object_get(json_object_get(j_user, "user"), "name")), GLEWLWYD_AUHORIZATION_TYPE_AUTHORIZATION_CODE_ACCESS_TOKEN, ip_source, scope_list, now);
        if (refresh_token != NULL) {
          access_token = generate_access_token(config, refresh_token, json_string_value(json_object_get(json_object_get(j_user, "user"), "login")), json_string_value(json_object_get(json_object_get(j_user, "user"), "name")), GLEWLWYD_AUHORIZATION_TYPE_AUTHORIZATION_CODE_ACCESS_TOKEN, ip_source, scope_list, json_object_get(json_object_get(j_user, "user"), "additional_property_name")!=json_null()?json_string_value(json_object_get(json_object_get(j_user, "user"), "additional_property_name")):NULL, json_object_get(json_object_get(j_user, "user"), "additional_property_value")!=json_null()?json_string_value(json_object_get(json_object_get(j_user, "user"), "additional_property_value")):NULL, now);
...
int check_auth_type_resource_owner_pwd_cred (const struct _u_request * request, struct _u_response * response, void * user_data) {
  ...
  if (check_result_value(j_auth, G_OK) || (request->auth_basic_user == NULL && request->auth_basic_password == NULL)) {
    ...
    if (check_result_value(j_result, G_OK)) {
      ...
      if (check_result_value(j_user, G_OK)) {
        time(&now);
        refresh_token = generate_refresh_token(config, request->auth_basic_user, json_string_value(json_object_get(json_object_get(j_user, "user"), "login")), json_string_value(json_object_get(json_object_get(j_user, "user"), "name")), GLEWLWYD_AUHORIZATION_TYPE_RESOURCE_OWNER_PASSWORD_CREDENTIALS, ip_source, json_string_value(json_object_get(j_result, "scope")), now);
        if (refresh_token != NULL) {
          access_token = generate_access_token(config, refresh_token, json_string_value(json_object_get(json_object_get(j_user, "user"), "login")), json_string_value(json_object_get(json_object_get(j_user, "user"), "name")), GLEWLWYD_AUHORIZATION_TYPE_RESOURCE_OWNER_PASSWORD_CREDENTIALS, ip_source, json_string_value(json_object_get(j_result, "scope")), json_object_get(json_object_get(j_user, "user"), "additional_property_name")!=json_null()?json_string_value(json_object_get(json_object_get(j_user, "user"), "additional_property_name")):NULL, json_object_get(json_object_get(j_user, "user"), "additional_property_value")!=json_null()?json_string_value(json_object_get(json_object_get(j_user, "user"), "additional_property_value")):NULL, now);
          if (access_token != NULL) {
              json_body = json_pack("{sssssssisi}",
                                    "token_type",
                                    "bearer",
